    ...
    searchBar.addEventListener("input", function() {
        ...
        this.searchTimer = setTimeout(function() {
            if (searchBar.value.length > 0) {
                logAuditTrail('SEARCH_ATTENDANCE', `Searched faculty attendance records: ${searchBar.value}`);
            }
            filterTable();
        }, 1000); 
    });
    ...
    document.querySelector(".btn-primary").addEventListener("click", function() {
        logAuditTrail('GENERATE_REPORT', 'Generated faculty attendance report');
        cleanupPrintElements();
                ...
                printContent.innerHTML = `
                    <div style="margin: 0; padding: 0; background-color: white;">
                        ...
                        <div class="report-content" style="text-align: center; font-family: Arial, sans-serif; background-color: white; padding: 20px; margin: 0;">
                            ...
                            <h2 style="position: relative; z-index: 2; margin-top: 20px;">SUMMARY OF FACULTY LIBRARY STATISTICS</h2>
    ...
    function logPageAccess() {
        logAuditTrail('ATTENDANCE_ACCESS', 'Accessed faculty attendance page');
    }
    ...
    function displayData(data) {
        ...
        if (displayData.length === 0) {
            tableBody.innerHTML = `<tr class="empty-message"><td colspan="7" class="text-center">No records found</td></tr>`;
            updateEntriesMessage(0, filteredData.length);
            return;
        }
        
        displayData.forEach(faculty => {
            const timeIn = faculty.time_in ? formatDateTime(faculty.time_in) : '-';
            const timeOut = faculty.time_out ? formatDateTime(faculty.time_out) : '-';
            const logDate = faculty.log_date ? formatDate(faculty.log_date) : '-';
    ...
    function applyFilters(data) {
        ...
        return data.filter(faculty => {
            ...
            const matchesProgram = program === '' || faculty.program === program;
            let matchesLogDate = true;
            if (logDateFilter) {
                const facultyLogDate = faculty.log_date ? formatDate(faculty.log_date) : '';
                matchesLogDate = facultyLogDate === logDateFilter;
            }
            let matchesReportDate = true;
            if (reportTypeValue !== "semester") {
                const logDate = faculty.log_date ? new Date(faculty.log_date) : null;
            ...
            if (schoolYear) {
                const facultyYear = faculty.log_date ? new Date(faculty.log_date).getFullYear() : null;
                matchesSchoolYear = facultyYear == schoolYear;
            }
            ...
            return matchesSearch && matchesDepartment && matchesProgram && matchesLogDate && matchesReportDate && matchesSchoolYear && matchesSemester;
        });
    }
